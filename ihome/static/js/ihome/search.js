...
function updateHouseData(action) {
    ...
    $.get("/api/v1.0/houses", params, function(resp){
        ...
        if ("0" == resp.errorno) {
            	if (0 == resp.data.max_pages) {
                	$(".house-list").html("暂时没有符合您查询的房屋信息。");
            	} else {
                	total_page = resp.data.max_pages;
                	if ("renew" == action) {
                		cur_page = 1;
            ...
            if (areaId) {
                ...
                for (var i=0; i<data.areas.length; i++) {
                    ...
                    if (data.areas[i].aid == areaId) {
                        $(".filter-area").append('<li area-id="'+ data.areas[i].id+'" class="active">'+ data.areas[i].name+'</li>');
                    } else {
                        $(".filter-area").append('<li area-id="'+ data.areas[i].id+'">'+ data.areas[i].name+'</li>');
                    }
                }
            } else {
